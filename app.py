...
st.title("💰 智能自动发票生成器")
st.markdown("无需上传，直接选择模板 -> 智能凑数 -> 下载 Word & PDF")
...
with st.sidebar:
    ...
    st.divider()
    # 5. PDF 选项
    generate_pdf = st.checkbox("同时生成 PDF 文件 (需要本地安装Word)", value=True)

# --- 主区域 ---

# 检查本地模板是否存在
if not os.path.exists(template_file):
    st.error(f"❌ 找不到本地模板文件：`{template_file}`，请确保它和代码放在同一个文件夹！")
    st.stop()
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    if generate_pdf:
                        try:
                            ...
                            # Streamlit 多线程环境下，调用 Windows COM 必须初始化
